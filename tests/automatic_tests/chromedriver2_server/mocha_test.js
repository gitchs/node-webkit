...
describe('chromedriver2_server', function() {
    ...
    before(function(done) {
        func.copyExecFiles(function() {
            //chromedriver2_server needs to be put under folder 'nw/tests/automatic_tests/chromedriver'
            var src_file;
            if (process.platform == 'win32')
                src_file = path.join(global.tests_dir, 'chromedriver2_server', 'chromedriver.exe');
            else
                src_file = path.join(global.tests_dir, 'chromedriver2_server', 'chromedriver');
            if (!fs.existsSync(src_file)) {
                isFileExist = false;
                done("chromedriver2_server needs to be put under folder 'nw/tests/automatic_tests/chromedriver'");
            }
